...
/*Own headers*/
#include "PublishTableController_mock.h"
/**/
...
*
*	@return	PTC_DB_STATUS_CODE	Status code for the database connection
*
*/
...
*
*	@return					PTC_DB_STATUS_CODE
*
*/
PTC_DB_STATUS_CODE  PublishTableController::executeQuery(QSqlDatabase connectionName, const string &strQuery, map<string, string> &arguments)
{
	QVariantMap args;
	QString key;
	QString value;
	...
	if (connectionName.isOpen())
	{
		...
		if (query->prepare(sql))
		{
			...
			if (!query->exec())
			{
				std::cout << "[PTC::Update Query]::Failed to execute query with errorcode - "
					<< query->lastError().text().toStdString() << endl;
				delete query;
				return PTC_DB_QUERY_FAILURE;
			}
		}
		else
		{
			std::cout << "[PTC::Update Query]::Failed to create prepared statement with errorcode - "
				<< query->lastError().text().toStdString() << endl;
			delete query;
			return PTC_DB_QUERY_FAILURE;
		}
		delete query;
		return PTC_DB_QUERY_SUCCESS;
	}
	return PTC_DB_QUERY_FAILURE;
}
...
*
*/
PTC_DB_STATUS_CODE PublishTableController::execSelectQuery(const string &strQuery, map<string, string> &arguments, QList<QMap<QString, QVariant>> &records)
{
	QVariantMap args;
	QString key;
	QVariant value;
	...
	if (m_ptcSelectCtn.isOpen())
	{
		QSqlQuery *query = new QSqlQuery(m_ptcSelectCtn);
		if (query->prepare(sql))
		{
			foreach(QString key_index, args.keys()) query->bindValue(key_index, args.value(key_index));
			if (query->exec())
			...
			{
				std::cout << "[PTC::Select Query]::Failed to execute query with errorcode - "
					<< query->lastError().text().toStdString() << endl;
				delete query;
				return PTC_SELECT_QUERY_FAILURE;
			}
		}
		else
		{
			std::cout << "[PTC::Select Query]::Failed to create prepare statement with errorcode - "
				<< query->lastError().text().toStdString() << endl;
			delete query;
			return PTC_SELECT_QUERY_FAILURE;
		}
		...
		delete query;
		return PTC_SELECT_QUERY_SUCCESS;
	}
	return PTC_SELECT_QUERY_FAILURE;
}
...
void PublishTableController::PublishTableInsertion(string passjson, string sensor)
{
	...
	if (PTC_DB_QUERY_FAILURE == executeQuery(m_ptcInsertCtn, dataInsertionQuery, args_dataInsertion))
	{
...
void PublishTableController::PublishTableInsertion(string passjson, string sensor, pair<string, string> send_receive)
{
	...
	if (sensor == "WIFI")
	{
		...
		if (PTC_DB_QUERY_FAILURE == executeQuery(m_ptcDeleteCtn, dataDeletionQueryWifi, args_dataDeletionWifi))
		{
	...
	if (sensor == "BLE")
	{
		...
		if (PTC_DB_QUERY_FAILURE == executeQuery(m_ptcDeleteCtn, dataDeletionQueryBLE, args_dataDeletionBLE))
		{
...
void PublishTableController::PublishControllerUpdatingBLE()
{
	...
	if (PTC_SELECT_QUERY_SUCCESS == execSelectQuery(dataRetrieveQuery, args_dataRetrieve, recs))
	{
...
void PublishTableController::PublishControllerUpdatingPIR()
{
	...
	if (PTC_SELECT_QUERY_SUCCESS == execSelectQuery(dataRetrieveQuery, args_dataRetrieve, recs))
	{
		...
		if (recs.size() > 0)
		{
			...
			if (PTC_DB_QUERY_FAILURE == executeQuery(m_ptcDeleteCtn, dataDeletionQuery, args_dataDeletion))
			{
...
pair<string, qlonglong> PublishTableController::PublishControllerUpdatingVendingDB()
{
	...
	{		
		...
		if (PTC_SELECT_QUERY_FAILURE == execSelectQuery(dataRetrieveQuery, args_dataRetrieve, recs))
		{
		...
		json_pair.insert("auditinfo", pair_value.first);							// inserting the event info array into the json key value pair
		...
		json_pair.insert("stockinfo", pair_value.first);						   // inserting the event info array into the json key value pair
		...
		json_pair.insert("vminfo", pair_value.first);			                  // inserting the event info array into the json key value pair
		...
		json_pair.insert("product_vend_info", pair_value.first);                 // inserting the event info array into the json key value pair
...
void PublishTableController::UpdatePublishTable()
{
	...
	if (!isFirst)
	{
		...
		if (m_ptcUpdateCtn.isOpen())
		{
			...
			if (query->prepare(TableCreationQuery))
			{
				if (!query->exec())
				{
					std::cout << "[DTC::UpdatePublishTable]::Failed to execute query with errorcode - "
						<< query->lastError().text().toStdString() << endl;					
				}
			}
			else
			{
				std::cout << "[DTC::UpdatePublishTable]::Failed to create prepared statement with errorcode - "
					<< query->lastError().text().toStdString() << endl;				
			}
