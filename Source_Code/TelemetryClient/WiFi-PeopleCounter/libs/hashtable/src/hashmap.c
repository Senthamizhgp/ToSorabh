...
#define HMAP_NULL_CHECK(val)  if (val == NULL) return HMAP_E_INVAL; /**< Check for NULL @hideinitializer */
...
typedef struct _hashmap_elem_t{
  uint8_t       *key;   /* pointer to actual key storage */
 ...
 */
static unsigned long crc32(const uint8_t *s, size_t len)
{
  uint32_t i;
 ...
 */
static uint32_t _find_hash_index(hashmap_map_t * m, const uint8_t* keystring){
...
/**
 * Return the integer of the location in data to store the point32_t to the item,
 *   or HMAP_E_OVERFLOW.
 *
 *   TODO:
 */
static int32_t _hashmap_hash(hmap_t in, uint8_t* key) {
  int32_t curr;
  int32_t i;
  hashmap_elem_t *elem;
 ...
 *
 * param[in]  handle to hashmap.
 * param[out] Nil.
 *
 * returns    HMAP_E_OUTMEM.
 *            HMAP_S_OK.
 */
static int32_t _hashmap_rehash(hmap_t in){
  int32_t i;
  int32_t old_size;
  hashmap_elem_t *curr;
  hashmap_elem_t *elem;
...
/**
 * @brief Create an empty hashmap of size HMAP_INITIAL_SIZE
 *
 * @return Pointer to the hashmap
 */
hmap_t hashmap_create()
{
	hashmap_map_t* m = (hashmap_map_t*) calloc(1, sizeof(hashmap_map_t));
	if (!m)
		return NULL;
 ...
 *
 * @param in    HashMap pointer
 * @param key   Key of the hashmap element
 * @param value Data corresponding to key to be added
 *
 * @return HMAP_S_OK for success
 */
int32_t hashmap_put(hmap_t in, uint8_t* key, void_ptr value)
{
	int32_t index;
	hashmap_map_t *m;
	hashmap_elem_t *elem;
...
/**
 * @brief Get the data for the particular key from the hashmap
 *
 * @param in   HashMap pointer
 * @param key  Key of the hashmap element
 * @param value Data corresponding to key to be fetched
 *
 * @return HMAP_S_OK for success
 * @return HMAP_E_NOTFOUND for key not found
 */
int32_t hashmap_get(hmap_t in, const uint8_t* key, void_ptr *value){
  int32_t curr;
  int32_t i;
  hashmap_map_t *m;
  hashmap_elem_t *elem;
...
/**
 * @brief
 * Iterate the callback function parameter over each element in the hashmap.
 * The void_ptr argument is passed as an argument to the callback function
 * The function returns anything other than HMAP_S_OK
 * the traversal is terminated. fn must not modify any hashmap functions.
 *
 * @param in		HashMap pointer
 * @param fnIterValue	User defined callback function to decipher the data
 * @param arg		Data pointer of the hashmap element.
 *
 * @return HMAP_S_OK for success
 */
int32_t hashmap_iterate(hmap_t in, hmap_callback_func fnIterValue, void_ptr arg)
{
	...
	for (i = 0; i< m->table_size; i++)
	{
		...
		if(elem->used == used_1)
		{
			int32_t status = fnIterValue(elem->data, arg);
			if (status != HMAP_S_OK)
				return status;
		}
	}
...
/**
 * @brief Remove the hashmap element corresponding the particular key
 *
 * @param in       HashMap pointer
 * @param key      Key of the hashmap element
 * @param outValue Data corresponding to key that holds the removed
 * element data.
 *
 * @return HMAP_S_OK for success
 * @return HMAP_E_NOTFOUND for key not found
...
/**
 * @brief De-allocate the hashmap
 *
 * @param in		HashMap pointer
 * @param fnFreeValue	User defined callback function to free the data
 * @param arg		Data pointer of the hashmap element.
 */
 ...
 * @param in HashMap pointer
 *
 * @return Number of hashmap entries
 */
