...
TRANSACTION_SUMMARY_STATUS_CODE Transaction::Transaction_Summary(string JsonData)
{
	...
	if (ProductVend.isArray())
	{
		...
		//	PVTime = Productobj["Timestamp"].toInt();
			...
			ProductID = Productobj["Product_ID"].toString();
...
//	Process_All_Transaction(JsonData);
	...
	if (StockObj.isArray())
	{
		...
		foreach(const QJsonValue & value, StockArray)
		{
			...
			PStockID = PStockObj["Product_ID"].toString().toStdString();
			...
			for (ProductinfoIter1 = ProductMap1.begin(); ProductinfoIter1 != ProductMap1.end(); ProductinfoIter1++)
			{
				...
				{
					localProductID = ProductinfoIter1->first.c_str();
			...
			//		cout << "Vendor Id	: " << ProdInnerMapIter1->second.toString().toUtf8().constData() << endl;
					dexBuffer.append("vendorId \t: " + ProdInnerMapIter1->second.toString() + "\n");
					innermap.insert("vendorId", ProdInnerMapIter1->second.toString());

			//		cout << "Product ID	: " << localProductID.toString().toStdString() << endl;
					dexBuffer.append("productId \t: " + localProductID + "\n");
					innermap.insert("productId", ProductinfoIter1->first.c_str());
			...
			//		cout << "productSalesCount : " << ProdInnerMapIter1->second.toString().toUtf8().constData() << endl;
					...
					innermap.insert("productSalesCount", ProdInnerMapIter1->second.toInt());

					ProdInnerMapIter1 = ProductinfoIter1->second.find("SalePrice");
					localProductPrice = ProdInnerMapIter1->second.toFloat();
			...
			//		innermap.insert("productPrice", ProdInnerMapIter1->second.toString());
					innermap.insert("productPrice", ProdInnerMapIter1->second.toFloat());
					...
					case 0:
				//		CoinSalesCount += 1;
						TotalCoinSales = (localProductPrice* static_cast<float> (localProductStock));
						cout << "Coin sales :: " << TotalCoinSales << endl;
						break;
					case 1:
				//		CardSalesCount += 1;
						TotalCardSales = (localProductPrice* static_cast<float> (localProductStock));
						break;
					case 2:
				//		CashSalesCount += 1;
						TotalCashSales = (localProductPrice*static_cast<float> (localProductStock));
						cout << "Cash sales :: " << TotalCashSales << endl;
						break;
					default:
						break;
					}
			...
			//		cout << "productCashlessSalesCount : " << ProdInnerMapIter1->second.toInt() << endl;
					...
					innermap.insert("productCashlessSalesCount", ProdInnerMapIter1->second.toInt());
			...
			//		cout << "ProductCurrentLevel : " << CurrentStock << endl;
					...
					innermap.insert("productCurrentLevel", CurrentStock);
					dexBuffer.append("\n");
					TotalCardSales = 0;					
				}
...
/**
------------------------------------------------------------------------------ -
FloattoStringConvertion
------------------------------------------------------------------------------ -
*	@brief		This function converts float format to string.
*
*	@param		float value
*
*	@return		char_string
*
*/
QString Transaction::FloattoStringConvertion(double Value)
{
	std::string ToString;
	QString Tochar;
	ToString = std::to_string(Value);				// to convert int to string
